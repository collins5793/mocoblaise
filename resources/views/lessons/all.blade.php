...
@section('title', 'Gestion de toutes les leçons')

@section('content')
<h2>Gestion de toutes les leçons</h2>

<form method="GET" action="{{ route('lessons.all') }}" style="margin-bottom: 1rem;">
    ...
    <a href="{{ route('lessons.create') }}" 
       style="float:right; background:#28a745; color:white; padding:0.5rem 1rem; border-radius:5px; text-decoration:none;">
       ➕ Ajouter une leçon
    </a>
</form>
...
<table style="width:100%; border-collapse: collapse; background:#fff; box-shadow: 0 0 10px rgb(0 0 0 / 0.1); border-radius:10px; overflow:hidden;">
    ...
    <tbody>
                ...
                <td>
                    @if($lesson->quizzes()->count() > 0)
                        <a href="{{ route('quizzes.index', ['lesson' => $lesson->id]) }}" 
                           style="background:#17a2b8; color:white; padding:0.3rem 0.6rem; border-radius:5px; text-decoration:none; margin-right:5px;">
                           Voir les quiz
                        </a>
                    @endif
        ...
        @empty
            <tr><td colspan="5" style="padding:1rem; text-align:center;">Aucune leçon trouvée.</td></tr>
        @endforelse
    </tbody>
</table>
