...
<div style="max-width:600px; margin: 3rem auto; background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 4px 10px rgb(0 0 0 / 0.1); font-family: 'Segoe UI', Tahoma, sans-serif; color: #1e293b;">
  ...
  <h1 style="font-weight: 700; font-size: 1.8rem; margin-bottom: 1.5rem; color: #334155;">Modifier le Quiz</h1>
  ...
  <form action="{{ route('quizzes.update', $quiz->id) }}" method="POST" style="display:flex; flex-direction: column; gap: 1.25rem;">
    @csrf
    @method('PUT')

    <input type="text" name="title" placeholder="Titre" value="{{ old('title', $quiz->title) }}" required
      style="padding: 0.6rem 1rem; font-size: 1rem; border: 1.5px solid #cbd5e1; border-radius: 6px; transition: border-color 0.3s;">

    <input type="number" name="duration_minutes" placeholder="Durée (minutes)" min="1" value="{{ old('duration_minutes', $quiz->duration_minutes) }}" required
      style="padding: 0.6rem 1rem; font-size: 1rem; border: 1.5px solid #cbd5e1; border-radius: 6px; transition: border-color 0.3s;">
    ...
    <select name="lesson_id" required
      ...
      @foreach ($lessons as $lesson)
        <option value="{{ $lesson->id }}" {{ $lesson->id == old('lesson_id', $quiz->lesson_id) ? 'selected' : '' }}>{{ $lesson->title }}</option>
      @endforeach
    </select>
